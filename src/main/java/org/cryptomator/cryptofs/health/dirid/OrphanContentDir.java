...
public class OrphanContentDir implements DiagnosticResult {
	...
	private void fix(Path pathToVault, VaultConfig config, Cryptor cryptor) throws IOException {
		...
		try (var nonCryptomatorFiles = Files.newDirectoryStream(orphanedDir)) {
			for (Path p : nonCryptomatorFiles) {
				Files.move(p, stepParentDir.contentDirPath().resolve(p.getFileName()), LinkOption.NOFOLLOW_LINKS);
			}
		}
	...
	// visible for testing
	CipherDir prepareStepParent(Path dataDir, Path cipherRecoveryDir, Cryptor cryptor, String clearStepParentDirName) throws IOException {
		...
		var stepParentCipherDir = new CipherDir(stepParentUUID, stepParentDir);
	...
	// visible for testing
	void adoptOrphanedResource(Path oldCipherPath, String newClearName, boolean isShortened, CipherDir stepParentDir, FileNameCryptor cryptor, MessageDigest sha1) throws IOException {
		...
		if (isShortened) {
			...
			Path targetPath = stepParentDir.contentDirPath().resolve(deflatedName);
			...
			Path targetPath = stepParentDir.contentDirPath().resolve(newCipherName);
			Files.move(oldCipherPath, targetPath);
		}
	}
